...
import { AppHotkeyId, AppHotkeys } from '../lib/app-hotkeys'
import {
...
export interface SettingsStoreState {
  ...
  setIsTrayIconHidden: (isHidden: boolean) => void
  ...
  setAppHotkey: (id: AppHotkeyId, hotkey: string) => void
}
...
export const settingsStore = createStore<SettingsStoreState & Settings>()((set, get) => ({
  ...
  updateSetting: async (name: string, value: string | boolean | number | null) => {
    ...
    try {
      ...
      if (name === 'appHotkeys' && typeof value === 'string') {
        const parsedHotkeys = JSON.parse(value)
        return set(() => ({
          appHotkeys: parsedHotkeys,
        }))
      }
  ...
  setIsTrayIconHidden: async (isHidden: boolean) => {
    const result = await get().updateSetting('isTrayIconHidden', isHidden)
    await invoke('set_tray_icon_hidden', { isHidden })
    return result
  },
  ...
  setAppHotkey: async (id: AppHotkeyId, hotkey: string) => {
    const appHotkeys = {
      ...get().appHotkeys,
      [id]: hotkey,
    }
    get().syncStateUpdate('appHotkeys', JSON.stringify(appHotkeys))
    return get().updateSetting('appHotkeys', JSON.stringify(appHotkeys))
  },
  ...
  setQuickPasteAcrylicOpacity: async (opacity: number) => {
    const normalizedOpacity = Math.min(100, Math.max(25, Math.round(opacity)))
  ...
  setQuickPasteHighlightColor: async (color: string) => {
    ...
    return get().updateSetting('quickPasteHighlightColor', color)
  },
...
export const listenToSettingsStoreEvents = listen('settings-store-sync', async event => {
  ...
  if (
    ...
    if (setting === 'appHotkeys' && typeof value === 'string') {
      settingsStore.setState({ appHotkeys: JSON.parse(value) })
      return
    }
    if (
      setting === 'quickPasteAcrylicOpacity' &&
      settingsStore.getState().quickPasteAcrylicOpacity !== value &&
      typeof value === 'number'
    ) {
      settingsStore.setState({ quickPasteAcrylicOpacity: value })
      return
    }
    if (
    ...
    if (
      setting === 'quickPasteHighlightColor' &&
      settingsStore.getState().quickPasteHighlightColor !== value &&
      typeof value === 'string'
    ) {
      settingsStore.setState({ quickPasteHighlightColor: value })
      return
    }
  }
})
