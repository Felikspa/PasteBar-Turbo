...
const DEFAULT_PASTE_SEQUENCE_SEPARATOR = '\n'
const displaySeparatorValue = (separator: string) =>
  separator.replace(/\n/g, '\\n').replace(/\t/g, '\\t')
const parseSeparatorValue = (separator: string) =>
  separator.replace(/\\n/g, '\n').replace(/\\t/g, '\t')
                  ...
                  <>
                    ...
                    <Box className="max-w-xl animate-in fade-in mt-4">
                      <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1">
                          <CardTitle className="animate-in fade-in text-md font-medium w-full">
                            {t('Multi-item paste separator', { ns: 'settings2' })}
                          </CardTitle>
                        </CardHeader>
                        <CardContent>
                          <Text className="text-sm text-muted-foreground mb-4">
                            {t(
                              'Inserted between adjacent items when pasting multiple history items from Quick Paste. Use \\n for a new line and \\t for a tab.',
                              { ns: 'settings2' }
                            )}
                          </Text>
                          <InputField
