...
import { hoveringHistoryRowId } from '~/store'
...
interface ClipboardHistoryQuickPasteRowProps {
  ...
  isQuickPasteDigitHighlighted?: boolean
  isCopied?: boolean
  isPasted?: boolean
  isSaved?: boolean
  isLargeView?: boolean
...
export function ClipboardHistoryQuickPasteRowComponent({
  ...
  isQuickPasteDigitHighlighted = false,
  ...
  const showCopyPasteIndexNumber = typeof index !== 'undefined' && index < 10
  ...
  return (
    ...
    >
      <Box ref={rowRef} tabIndex={0} role="option" aria-selected={isKeyboardSelected}>
        ...
        >
          <Box
            data-keyboard-selected={isKeyboardSelected ? 'true' : 'false'}
            data-quick-paste-digit-highlighted={
              isQuickPasteDigitHighlighted ? 'true' : 'false'
            }
            className={`quick-paste-row-surface rounded-md justify-center relative px-3 py-0 my-0 shadow-none border-0 flex flex-col ${
              index === 0 &&
          ...
          >
            <Box className={`${showSelectHistoryItems ? 'flex flex-row -ml-1' : ''}`}>
              ...
              {showCopyPasteIndexNumber && (
                <Box className="quick-paste-hotkey-area pointer-events-none">
                  <Badge
                    className="quick-paste-index-badge font-mono bg-slate-200 dark:bg-slate-700 !py-0"
                    variant="outline"
                  >
                    {index === 9 ? 0 : index + 1}
                  </Badge>
