...
export function NavBar() {
  ...
  useHotkeys(getAppHotkey(appHotkeys, 'navHistory'), () => {
    navigate('/history', { replace: true })
  })

  useHotkeys(getAppHotkey(appHotkeys, 'navMenu'), () => {
    navigate('/menu', { replace: true })
  })

  useHotkeys(getAppHotkey(appHotkeys, 'lockApp'), () => {
    isAppLocked.value = true
  })

  useHotkeys(getAppHotkey(appHotkeys, 'swapPanels'), () => {
    setIsSwapPanels(!isSwapPanels)
  })

  useHotkeys(getAppHotkey(appHotkeys, 'mediaPlayPause'), e => {
    e.preventDefault()
  ...
  useHotkeys(getAppHotkey(appHotkeys, 'mediaNext'), () => {
    if (playerSongs.length > 0) {
      handleNext()
    }
  })

  useHotkeys(getAppHotkey(appHotkeys, 'mediaPrev'), () => {
    if (playerSongs.length > 0) {
      handlePrev()
    }
  })

  useHotkeys(getAppHotkey(appHotkeys, 'settingsCollections'), () => {
    navigate('/app-settings/collections', { replace: true })
  })

  useHotkeys(getAppHotkey(appHotkeys, 'settingsHistory'), () => {
    navigate('/app-settings/history', { replace: true })
  })

  useHotkeys(getAppHotkey(appHotkeys, 'settingsPreferences'), () => {
    navigate('/app-settings/preferences', { replace: true })
  })

  useHotkeys(getAppHotkey(appHotkeys, 'closeApp'), () => {
    if (isWindows) {
      closeWindow()
    }
    if (!isWindows) {
  ...
  useHotkeys(getAppHotkey(appHotkeys, 'toggleSplitView'), async () => {
  ...
  return (
    <div
      ...
      className="h-[41px] absolute top-0 left-0 w-full bg-white dark:bg-[#0f0f11]"
      onMouseEnter={() => {
    ...
    >
      <Menubar
        data-tauri-drag-region
        className={`border-0 !h-full _border-b _border-gray-200/50 _dark:border-gray-500/50 rounded-b-none bg-white pl-3 hover:bg-white dark:hover:bg-[#0f0f11] active:cursor-move active:bg-white transform duration-300 dark:bg-[#0f0f11] dark:text-slate-300`}
      >
