...
function App() {
  ...
  const registeredGlobalHotkeysRef = useRef<string[]>([])
  ...
  useEffect(() => {
    if (!window.isMainWindow || !settingsStore.isAppReady) {
      return
    }

    let isDisposed = false

    const bindGlobalHotkeys = async () => {
      for (const hotkey of registeredGlobalHotkeysRef.current) {
        await unregister(hotkey).catch(console.error)
      }

      const registeredHotkeys: string[] = []
      const mainWindowHotkey = settingsStore.hotKeysShowHideMainAppWindow?.trim()
      const quickPasteHotkey = settingsStore.hotKeysShowHideQuickPasteWindow?.trim()

      if (mainWindowHotkey) {
        await register(mainWindowHotkey, async () => {
        ...
        registeredHotkeys.push(mainWindowHotkey)
      }
      ...
      registeredGlobalHotkeysRef.current = registeredHotkeys
    }
    ...
    settingsStore.hotKeysShowHideMainAppWindow,
    settingsStore.hotKeysShowHideQuickPasteWindow,
