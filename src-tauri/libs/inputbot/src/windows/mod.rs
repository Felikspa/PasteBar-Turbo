...
pub fn handle_input_events() {
  ...
  if !KEYBD_BINDS.lock().unwrap().is_empty() || !KEYBD_RELEASE_BINDS.lock().unwrap().is_empty() {
...
unsafe extern "system" fn keybd_proc(code: c_int, w_param: WPARAM, l_param: LPARAM) -> LRESULT {
  if KEYBD_BINDS.lock().unwrap().is_empty() && KEYBD_RELEASE_BINDS.lock().unwrap().is_empty() {
    ...
    let binds = match w_param as u32 {
      WM_KEYDOWN | WM_SYSKEYDOWN => Some(&KEYBD_BINDS),
      WM_KEYUP | WM_SYSKEYUP => Some(&KEYBD_RELEASE_BINDS),
      _ => None,
    };
