...
static INTERNAL_CLIPBOARD_WRITE_COUNT: AtomicUsize = AtomicUsize::new(0);
const QUICKPASTE_CLIPBOARD_SETTLE_DELAY_MS: u64 = 20;
...
pub fn mark_internal_clipboard_write() {
  INTERNAL_CLIPBOARD_WRITE_COUNT.fetch_add(1, Ordering::SeqCst);
}

pub fn take_internal_clipboard_write() -> bool {
  let mut current = INTERNAL_CLIPBOARD_WRITE_COUNT.load(Ordering::SeqCst);

  while current > 0 {
    match INTERNAL_CLIPBOARD_WRITE_COUNT.compare_exchange(
      current,
      current - 1,
      Ordering::SeqCst,
      Ordering::SeqCst,
...
#[tauri::command]
pub fn quickpaste_copy_history_item(app_handle: AppHandle, history_id: String) -> String {
  mark_internal_clipboard_write();
  copy_history_item(app_handle, history_id)
}

#[tauri::command]
pub fn quickpaste_copy_history_items(
  ...
  if history_ids.len() == 1 {
    let Some(history_id) = history_ids.into_iter().next() else {
      return "ok".to_string();
    };

    return quickpaste_copy_history_item(app_handle, history_id);
  }
  ...
  mark_internal_clipboard_write();
  match manager.write_text(combined_value) {
    Ok(_) => "ok".to_string(),
    Err(e) => {
      eprintln!("Failed to write to clipboard: {}", e);
      "Failed to write to clipboard".to_string()
...
pub fn copy_paste_history_item_internal(
  app_handle: AppHandle,
  history_id: String,
  delay: i32,
) -> String {
...
pub fn copy_paste_history_items(
  app_handle: AppHandle,
  history_ids: Vec<String>,
  separator: String,
  prefix_separator: bool,
